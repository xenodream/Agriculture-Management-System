...
import { useGetLatestDaysQuery } from "../redux/slices/workApiSlice";
...
`;
const LatestCollectionDays = () => {
  const { data, isLoading, isError } = useGetLatestDaysQuery();
  ...
  return (
    <LatestCollections>
      <PanelSectionHeader>
        ...
        <PanelSectionButton>
          <StyledLink to="/work-days" onClick={handleClick}>
            <AddButtonSection>+</AddButtonSection>
          </StyledLink>
          <ButtonDescription>Pokaż wszystko</ButtonDescription>
        </PanelSectionButton>
        ...
        {formattedData.map((workDay) => {
          return (
            <PanelSectionRow key={workDay.id}>
              ...
              <PanelSectionData>
                {String(workDay.date).slice(0, 10)}
              </PanelSectionData>
              <PanelSectionData>{workDay.type_of_work}</PanelSectionData>
              <PanelSectionData>
                {Number(workDay.yields_weight).toFixed(2)} kg
              </PanelSectionData>
              <PanelSectionData>
...
export default LatestCollectionDays;
